...
export default function Reports() {
  ...
  const inputClass = (fKey: keyof AdvancedFilters, fKey2?: keyof AdvancedFilters) => {
    ...
    return `w-full px-4 py-2.5 rounded-xl outline-none transition-all text-sm font-medium border ${
      isActive 
      ? 'bg-[var(--alert-success-100)] border-[var(--alert-success-400)] text-[var(--alert-success-900)] ring-2 ring-[var(--alert-success-500)]/10' 
      : 'bg-[var(--white)] border-[var(--grey-200)] text-[var(--secondary-500)] focus:ring-2 focus:ring-[var(--primary-500)]/20'
    }`;
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="h4 text-[var(--secondary-500)] tracking-tight">التقارير المتقدمة</h2>
          <p className="body-4 text-[var(--grey-500)] mt-1">تخصيص التقارير الإغاثية باستخدام فلاتر متعددة للوصول الدقيق للبيانات.</p>
        </div>
      </div>

      {/* Quick Access Reports */}
      <div className="bg-gradient-to-l from-[var(--primary-50)] to-blue-50/50 rounded-2xl p-4 border border-[var(--primary-100)] hidden md:block">
        <div className="flex items-center gap-2 mb-3">
          <Zap className="w-5 h-5 text-[var(--primary-500)]" />
          <h3 className="caption font-bold text-[var(--secondary-500)]">تقارير سريعة بنقرة واحدة</h3>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
           <button onClick={() => applyQuickReport('noProjects')} className="flex items-center gap-2 px-3 py-2 bg-[var(--white)] hover:bg-[var(--bg-tertiary)] border border-[var(--grey-200)] rounded-lg transition-all shadow-sm group">
              <FolderX className="w-4 h-4 text-[var(--alert-danger-500)]" />
              <span className="caption font-bold text-[var(--secondary-500)]">بلا مشاريع</span>
           </button>
           <button onClick={() => applyQuickReport('hasProjects')} className="flex items-center gap-2 px-3 py-2 bg-[var(--white)] hover:bg-[var(--bg-tertiary)] border border-[var(--grey-200)] rounded-lg transition-all shadow-sm group">
              <FolderHeart className="w-4 h-4 text-[var(--alert-success-500)]" />
              <span className="caption font-bold text-[var(--secondary-500)]">مستفيدة</span>
           </button>
           <button onClick={() => { setShowFilters(true); document.getElementById('assigneeFilter')?.focus(); }} className="flex items-center gap-2 px-3 py-2 bg-[var(--white)] hover:bg-[var(--bg-tertiary)] border border-[var(--grey-200)] rounded-lg transition-all shadow-sm group">
              <User className="w-4 h-4 text-[var(--tertiary-500)]" />
              <span className="caption font-bold text-[var(--secondary-500)]">حسب المكلف</span>
           </button>
           <button onClick={() => { setShowFilters(true); document.getElementById('projectFilter')?.focus(); }} className="flex items-center gap-2 px-3 py-2 bg-[var(--white)] hover:bg-[var(--bg-tertiary)] border border-[var(--grey-200)] rounded-lg transition-all shadow-sm group">
              <Folder className="w-4 h-4 text-[var(--alert-warning-500)]" />
              <span className="caption font-bold text-[var(--secondary-500)]">مشروع معين</span>
           </button>
           <button onClick={() => applyQuickReport('largeFamilies')} className="flex items-center gap-2 px-3 py-2 bg-[var(--white)] hover:bg-[var(--bg-tertiary)] border border-[var(--grey-200)] rounded-lg transition-all shadow-sm group">
              <Users className="w-4 h-4 text-[var(--primary-500)]" />
              <span className="caption font-bold text-[var(--secondary-500)]">الأسر (6+)</span>
           </button>
        </div>
      </div>
      ...
      {/* Advanced Filters */}
      {showFilters && (
        <div className="card p-0 overflow-hidden animate-in fade-in slide-in-from-top-2">
          <div className="card-header pb-4 mb-0 border-b border-[var(--grey-100)] flex justify-between items-center px-5 py-5">
             <h3 className="card-title flex items-center gap-2"><Sparkles className="w-5 h-5 text-[var(--primary-500)]" /> خيارات التصفية</h3>
             {activeChips.length > 0 && (
               <button onClick={handleClearFilters} className="text-xs font-bold text-[var(--grey-500)] hover:text-[var(--alert-danger-500)] flex items-center gap-1.5 transition-colors">
          ...
          <div className="border-t border-[var(--grey-100)] p-5 bg-[var(--bg-tertiary)] relative">
             <div className="flex items-center gap-2 mb-3">
               <Settings2 className="w-4 h-4 text-[var(--grey-500)]" />
               <h4 className="caption font-bold text-[var(--secondary-500)]">تخصيص أعمدة العرض والتصدير:</h4>
             </div>
                  ...
                  return (
                    <button
                       ...
                       className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border shadow-sm ${
                          isActive 
                          ? 'bg-[var(--white)] border-[var(--primary-200)] text-[var(--primary-700)]' 
                          : 'bg-[var(--bg-tertiary)] border-[var(--grey-200)] text-[var(--grey-400)] opacity-60 hover:opacity-100 hover:bg-[var(--white)]'
                       }`}
                    >
      ...
      {/* Results Section */}
      {hasSearched && (
        <div className="card p-0 overflow-hidden animate-in fade-in duration-500 relative flex flex-col">
          
          {/* Active Filters Summary (Chip bar) */}
          {activeChips.length > 0 && (
             <div className="px-5 py-4 bg-[var(--primary-50)] border-b border-[var(--primary-100)]">
                <div className="flex flex-wrap items-center gap-2">
                   <span className="caption font-bold text-[var(--primary-800)] flex items-center gap-1.5 ml-2">
                      <FilterX className="w-4 h-4" /> الفلاتر المطبقة:
                   </span>
                   {activeChips.map(chip => (
                      <span key={chip.id} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-[var(--white)] border border-[var(--alert-success-200)] text-[var(--alert-success-900)] text-xs font-bold shadow-sm">
                         <span className="text-[var(--alert-success-600)]">{chip.label}:</span> {chip.value}
                         <button onClick={() => removeFilter(chip.clearKey)} className="hover:text-[var(--alert-danger-500)] hover:bg-[var(--alert-danger-50)] rounded-full p-0.5 ml-1 transition-colors">
                            <X className="w-3 h-3" />
                         </button>
                      </span>
                   ))}
                   
                   <button onClick={handleClearFilters} className="caption font-bold text-[var(--alert-danger-500)] hover:text-[var(--alert-danger-700)] hover:bg-[var(--alert-danger-50)] px-2 py-1.5 rounded-lg transition-colors mr-auto flex items-center gap-1">
                      مسح الكل
                   </button>
                </div>
             </div>
          ...
          <div className="p-4 sm:p-5 border-b border-[var(--grey-100)] bg-[var(--white)] flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 z-20">
              <div className="flex items-center gap-3">
                 <h3 className="h5 text-[var(--secondary-500)] tracking-tight">النتائج</h3>
                 <span className="px-2.5 py-1 bg-[var(--primary-100)] text-[var(--primary-700)] rounded-md caption font-bold leading-none shadow-sm">{filteredData.length} سجل مطبق</span>
              </div>
              
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => setShowFilters(!showFilters)} 
                  className="btn btn-outline h-9 px-3 text-sm py-0 bg-[var(--bg-tertiary)]"
                >
                  {showFilters ? <><ChevronUp className="w-4 h-4 mr-1.5"/> طي الفلاتر</> : <><ChevronDown className="w-4 h-4 mr-1.5"/> إظهار الفلاتر</>}
                </button>
                
                {filteredData.length > 0 && (
                  <button onClick={openExportModal} className="btn btn-outline border-[var(--alert-success-300)] text-[var(--alert-success-600)] hover:bg-[var(--alert-success-50)] shadow-sm h-9 px-4 py-0 text-sm">
          ...
          <div className="overflow-hidden flex flex-col border-t border-[var(--grey-100)]">
             {filteredData.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-[var(--grey-400)] bg-[var(--bg-tertiary)] min-h-[350px]">
                  <FilterX className="w-12 h-12 mb-3 text-[var(--grey-300)]" />
                  <p className="font-bold text-lg text-[var(--grey-600)]">لا يوجد بيانات مطابقة للفلاتر المحددة</p>
                  <p className="text-sm mt-1 text-[var(--grey-500)] max-w-sm text-center">قم بإلغاء بعض الفلاتر العلوية أو تغيير نطاقات البحث للحصول على نتائج</p>
                  <button onClick={handleClearFilters} className="mt-4 btn btn-outline h-9 px-4 py-0 text-sm">تصفير الفلاتر</button>
                </div>
              ) : (
              <>
              <div className="overflow-x-auto overflow-y-auto max-h-[500px]">
              <table className="table w-full text-right text-sm whitespace-nowrap relative min-w-[1000px] border-0 rounded-none shadow-none">
                <thead className="bg-[var(--bg-tertiary)] border-b border-[var(--grey-200)] text-[var(--secondary-500)] sticky top-0 z-20">
                  <tr>
                    {selectedColumns.includes('headFullName') && <th>اسم رب الأسرة</th>}
                    {selectedColumns.includes('headIdentityNumber') && <th className="text-center">رقم الهوية</th>}
                    {selectedColumns.includes('mobileNumber') && <th className="text-center">رقم الجوال</th>}
                    {selectedColumns.includes('memberCount') && <th className="text-center">عدد الأفراد</th>}
                    {selectedColumns.includes('region') && <th className="text-center">السكن الحالي</th>}
                    {selectedColumns.includes('assignee') && <th className="text-center">المكلف</th>}
                    {selectedColumns.includes('projectsCount') && <th className="text-center">مرات الاستفادة</th>}
                    {selectedColumns.includes('lastDisbursement') && <th className="text-center">تاريخ آخر صرف</th>}
                  </tr>
                </thead>
                <tbody className="bg-[var(--white)] relative z-0">
                    {paginatedData.map(f => {
                      const lastDisb = getFamilyLastDisbursement(f.id);
                      return (
                        <tr key={f.id}>
                          {selectedColumns.includes('headFullName') && <td className="font-bold text-[var(--secondary-500)]">{f.headFullName}</td>}
                          {selectedColumns.includes('headIdentityNumber') && <td className="font-mono text-[var(--grey-500)] text-xs tracking-wider text-center">{f.headIdentityNumber}</td>}
                          {selectedColumns.includes('mobileNumber') && <td className="font-mono text-[var(--grey-500)] text-xs tracking-wide text-center">{f.mobileNumber || '-'}</td>}
                          {selectedColumns.includes('memberCount') && <td className="text-center font-bold text-[var(--secondary-500)]">{f.memberCount + 1}</td>}
                          {selectedColumns.includes('region') && <td className="text-center">
                             <span className={`inline-flex px-2 py-1.5 rounded-md text-[10px] font-bold ${f.region === 'شمال وادي غزة' ? 'bg-[var(--alert-warning-100)] text-[var(--alert-warning-700)]' : f.region === 'جنوب وادي غزة' ? 'bg-[var(--primary-100)] text-[var(--primary-700)]' : 'bg-[var(--bg-tertiary)] text-[var(--grey-600)]'}`}>
                                {f.region || 'غير محدد'}
                             </span>
                          </td>}
                          {selectedColumns.includes('assignee') && <td className="text-center text-[var(--grey-600)] font-medium">{getAssigneeName(f.assigneeId)}</td>}
                          {selectedColumns.includes('projectsCount') && <td className="text-center">
                             <span className={`inline-flex items-center justify-center min-w-[2rem] px-1.5 py-1 rounded-md text-xs font-bold ${getFamilyProjectCount(f.id) > 0 ? 'bg-[var(--alert-success-100)] text-[var(--alert-success-800)]' : 'bg-[var(--bg-tertiary)] text-[var(--grey-400)]'}`}>
                               {getFamilyProjectCount(f.id)}
                             </span>
                          </td>}
                          {selectedColumns.includes('lastDisbursement') && <td className="text-center text-[var(--grey-500)] font-mono text-xs">
                            {lastDisb ? (
                               <span className="bg-[var(--bg-tertiary)] px-2.5 py-1 rounded border border-[var(--grey-200)] font-medium">{lastDisb}</span>
                            ) : (
              ...
              <div className="flex flex-wrap items-center justify-between p-4 border-t border-[var(--grey-100)] bg-[var(--bg-tertiary)] gap-4">
                  <span className="text-xs text-[var(--grey-500)] font-medium font-mono bg-[var(--white)] px-3 py-1.5 rounded border border-[var(--grey-200)] shadow-sm">
                     عرض {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredData.length)} من أصل {filteredData.length}
                  </span>
                  <div className="flex items-center gap-1.5">
                     <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="btn-pagination h-8">السابق</button>
                     <span className="text-xs font-extrabold text-[var(--secondary-500)] px-3 bg-[var(--white)] py-1.5 flex items-center rounded-lg border border-[var(--grey-200)] shadow-sm">صفحة {currentPage} / {totalPages}</span>
                     <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="btn-pagination h-8">التالي</button>
                  </div>
              </div>
              </>
              )}
      ...
      {isExportModalOpen && (
        <div className="fixed inset-0 bg-[var(--secondary-500)]/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-md relative animate-in zoom-in-95 duration-200">
            <button onClick={() => setIsExportModalOpen(false)} className="absolute top-6 left-6 p-2 text-[var(--grey-400)] hover:text-[var(--secondary-500)] hover:bg-[var(--bg-tertiary)] rounded-full transition-colors"><X className="w-5 h-5"/></button>
            
            <div className="card-header border-b-0 pb-0 mb-6">
               <h3 className="h5 text-[var(--secondary-500)] tracking-tight mb-1 flex items-center gap-2">
                 <Download className="w-5 h-5 text-[var(--alert-success-600)]" /> تصدير التقرير
               </h3>
               <p className="body-4 text-[var(--grey-500)]">سيتم استخراج البيانات بناءً على الفلاتر والأعمدة التي حددتها.</p>
            </div>
            
            <div className="space-y-5 mb-8">
              {/* Display Active Filters summary inside modal */}
              {activeChips.length > 0 ? (
                <div className="bg-[var(--primary-50)] border border-[var(--primary-100)] rounded-xl p-4">
                   <p className="caption font-bold text-[var(--primary-800)] mb-2">الفلاتر المضمنة في الاستخراج:</p>
                   <div className="flex flex-wrap gap-1.5">
                     {activeChips.map(chip => (
                        <span key={chip.id} className="px-2 py-1 bg-[var(--white)] border border-[var(--primary-100)] text-[var(--primary-700)] text-[10px] font-bold rounded">
                ...
                <div className="bg-[var(--bg-tertiary)] border border-[var(--grey-100)] rounded-xl p-4 text-center">
                   <p className="caption font-bold text-[var(--grey-500)]">سيتم استخراج كافة البيانات (لا يوجد فلاتر مطبقة)</p>
                </div>
              )}

              <div className="form-field">
                <label className="form-label mb-2">تسمية الملف (اقتراح ذكي)</label>
                <div className="relative">
                  <input 
                    ...
                    autoFocus
                    className="form-input pl-12 pr-4 py-3 h-[46px] w-full text-left font-bold" dir="ltr"
                  />
                  <span className="absolute left-4 top-3.5 caption font-bold text-[var(--grey-400)] pointer-events-none">.xlsx</span>
                </div>
              </div>
            </div>

            <div className="flex gap-3">
              <button onClick={performExport} className="btn border border-[var(--alert-success-600)] bg-[var(--alert-success-600)] text-[var(--white)] hover:bg-[var(--alert-success-700)] flex-1 h-[46px]">
                <Check className="w-5 h-5 mr-2"/> بدء التصدير القطعي
              </button>
              <button onClick={() => setIsExportModalOpen(false)} className="btn btn-outline min-w-[120px] h-[46px]">إلغاء</button>
            </div>
          </div>
        </div>
      )}
